...
/// query used to get jobs definitions from DB
const char* ORCL_JOB_DEFS = R""""(select sd.job_cd as master,  LEVEL, jb.job_cd, jb.next_job_cd_success
, case jb.job_typ_enu
    when 3 -- chain
    then jb.parm_1
    else ''
...
const char* by::Queries::getOrclJobDefQuery() {
  return ORCL_JOB_DEFS;
} 
