	...
	const Poco::AutoPtr<EntySelCta> esc_ptr, std::ofstream& r_out)
{
	...
	{
		// print out the /**...**/ comment from the ESC code
		r_out << getEscComments(esc_ptr) << "\n\n";

		// print out the ESC description
...
std::string asarum::BY::DocWriter::getEscComments(const Poco::AutoPtr<EntySelCta> esc_ptr) {
	std::string comment = "";
	if (esc_ptr->cta_sql().isNull() == false) {
		const std::string com_start = "/**";
		const std::string com_end = "*/";
		std::string sql{ esc_ptr->cta_sql() };

		size_t start_pos = sql.find(com_start);
		if (start_pos == std::string::npos)  return "";

		size_t end_pos = sql.find(com_end);
		if (end_pos == std::string::npos) return "";

		start_pos += com_start.length();
		comment = "#### Comment \n\n" + sql.substr(start_pos, end_pos - start_pos);
	}
	// if esc query is empty, return empty string
	return comment;
}
/***************************************************************/
