...
void by::DbCaller::getJobGroups(
    std::vector<std::shared_ptr<asarum::BY::JobGroup>> &job_groups) {
  soci::rowset<soci::row> rows =
      m_connector.getQueryData(by::Queries::getOrclJobDefQuery());
  std::shared_ptr<by::JobGroup> pGroup_ = nullptr;

  // soci uses indicators to indicate if a column is or is not NULL. If a
  // colummn returns a non null value, the function get_indicator(col_num)
  // returns soci::i_ok
  soci::indicator ind_;
  ...
  for (auto &row : rows) {
    ...
    if (ind_ == soci::i_ok) {
      ...
      std::shared_ptr<by::JobGroup> new_group{
          new by::JobGroup(row.get<std::string>(0))};
    ...
    // iterating through the list of columns and adding their values to the
    // fields vector
    ...
    for (auto i = 0; i < row.size(); i++) {
      ...
      if (ind_ == soci::i_ok) {
        ...
        if (i == 1)
          fields_.push_back(std::to_string(row.get<double>(i)));
        else
          fields_.push_back(row.get<std::string>(i));
