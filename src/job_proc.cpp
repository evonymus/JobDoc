...
void by::JobProc::readFile(const char *fileName, std::string &str) {
  ...
  if (myFile.is_open()) {
    ...
    str = std::string((std::istreambuf_iterator<char>(myFile)),
...
void by::JobProc::processFile(const char *fileName) {
  ...
  try {
    ...
    // creating pointer to JobGroup object. JobGroup is a group of jobs
    ...
    for (auto i = 1; i < jobs_.size(); i++) {
      ...
      if (jb_grp_ptr_ != nullptr) {
        // if the number of fields equalt to the number of fields
        if (fields.size() == FIELDS_NO) {
    ...
    std::cout << "unexptected exception occured\n";
  }
}
...
void by::JobProc::exportJobs(const std::string &path, const bool withSummary) {
  ...
  // processing sequentially every job group
  for (const auto &gr : m_job_groups) {
    std::string grp_dir = job_dir + "/" + gr->m_group_name;
    // creating directory for specific jobs group
    fs::create_directory(grp_dir);

    // iterating all the jobs beloging to the group
...
/// diagrams
void by::JobProc::exportDocs(const std::string &path, const bool withDiagram) {
...
/// @os  output stream
void by::JobProc::printGroupDescription(const JobGroup &gr,
                                        std::ostringstream &so) {
  so << "# " << gr.m_group_name << "\n\n"
     << "## Description\n\n"
...
void by::JobProc::printGroupTable(by::JobGroup &gr, std::ostringstream &sa) {
  ...
  sa << std::setfill('-') << "| " << std::setw(gr.m_len_lp) << "-" << " | "
     << std::setw(gr.m_len_job_cd) << "-" << " | " << std::setw(gr.m_len_sub)
...
void by::JobProc::printStepsDescriptions(const JobGroup &gr,
...
/// @param so ostringstream used by mother function for processing and
/// formatting data
///        here used for the same purpose.
void by::JobProc::printMermaidSequence(const JobGroup &gr,
                                       std::ostringstream &so) {
  using namespace std;
  ...
  if (gr.m_jobs.size() > 1) {
    ...
    for (auto &jb : gr.m_jobs) {
      if (!jb->m_next_job_success.empty()) {
        ...
        jb->m_next_job_success.erase(
            remove_if(jb->m_next_job_success.begin(),
